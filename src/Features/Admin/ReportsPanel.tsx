...
import { ApiError, fetchReportedQuizzes, setQuizStatus } from '@/Lib/Api/Client';
...
// Moderation: reported quizzes + block/unblock. Rendered inside AdminDashboard.
export default function ReportsPanel() {
  ...
  if (reports === null) {
    return <Card color="bg" className={styles.state}><p>Loading reports…</p></Card>;
  }
  if (reports.length === 0) {
    return <Card color="sage" className={styles.state}><p>No reported quizzes. All clear.</p></Card>;
  }
    ...
    <div className={styles.list}>
      {reports.map((report) => (
        ...
        >
          ...
          >
            {busy === report.quizId ? 'Saving…' : report.status === 'blocked' ? 'Unblock' : 'Block'}
          </Button>
        </Card>
      ))}
    </div>
